...
describe('Docs Generator', () => {
  ...
  describe('generate', () => {
    it('should generate markdown content', async () => {
      ...
      expect(content).toContain('Auto-generated by Sparn');
    });
    ...
    it('should include sparn optimization section when .sparn exists', async () => {
      mkdirSync(join(tmpDir, '.sparn'), { recursive: true });
      writeFile('.sparn/config.yaml', 'pruning:\n  threshold: 5');
      writeFile('package.json', JSON.stringify({ name: 'test', version: '1.0.0' }));
      ...
      expect(content).toContain('Sparn Optimization');
    });
