...
export function createDocsGenerator(config: DocsGeneratorConfig): DocsGenerator {
  ...
  async function generate(graph?: DependencyGraph): Promise<string> {
    ...
    lines.push(`<!-- Auto-generated by Cortex v1.0.0 — ${now} -->`);
    lines.push('');
    ...
    // Cortex config
    const cortexConfigPath = join(projectRoot, '.cortex', 'config.yaml');
    if (existsSync(cortexConfigPath)) {
      lines.push('## Cortex Optimization');
      lines.push('');
      lines.push('Cortex is active in this project. Key features:');
      lines.push('- Context optimization (60-70% token reduction)');
      lines.push('- Use `cortex search` before reading files');
      lines.push('- Use `cortex graph` to understand dependencies');
      lines.push('- Use `cortex plan` for planning, `cortex exec` for execution');
      lines.push('');
    }
